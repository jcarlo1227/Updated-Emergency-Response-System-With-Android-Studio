...
export async function createResponder(input, admin) {
    const existingByEmail = await Responder.findOne({ email: input.email });
    if (existingByEmail) {
        throw new AppError('A responder with this email already exists', 409, 'EMAIL_TAKEN');
    }
    if (input.phone) {
        const existingByPhone = await Responder.findOne({ phone: input.phone });
        if (existingByPhone) {
            throw new AppError('A responder with this phone number already exists', 409, 'PHONE_TAKEN');
        }
    }
    let assignedUnit;
