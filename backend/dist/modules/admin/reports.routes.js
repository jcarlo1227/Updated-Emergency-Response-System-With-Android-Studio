...
import { AmbulanceTransportRequest, AmbulanceUnit, Emergency, Responder, } from '../../models/index.js';
const router = Router();
...
router.get('/reports', validateRequest({ query: reportsQuerySchema }), async (req, res, next) => {
    try {
        const q = req.validated.query;
        const { from, to } = resolveRange(q);
        // Emergency filter
        const emFilter = { createdAt: { $gte: from, $lte: to } };
        if (q.type)
            emFilter.type = q.type;
        if (q.barangay)
            emFilter.barangay = q.barangay;
        if (q.responderId)
            emFilter.assignedResponderId = new mongoose.Types.ObjectId(q.responderId);
        // Ambulance filter
        const ambFilter = { createdAt: { $gte: from, $lte: to } };
        if (q.responderId)
        ...
        const [totalIncidents, emergenciesByStatus, emergenciesByType, topBarangays, responderActivity, ambulanceTotal, ambulanceByStatus, ambulanceUsage, monthlyEmergencies, monthlyAmbulance, responseAgg,] = await Promise.all([
        ...
        const pendingIncidents = (emStatuses.pending ?? 0) +
        ...
        for (const r of monthlyEmergencies) {
            ...
            if (r._id.type === 'medical')
                b.medical += r.count;
            else if (r._id.type === 'crime')
                b.crime += r.count;
            else if (r._id.type === 'fire')
                b.fire += r.count;
            else if (r._id.type === 'general_sos')
                b.general_sos += r.count;
        }
        for (const r of monthlyAmbulance) {
            const b = ensure(r._id.y, r._id.m);
            b.ambulance += r.count;
        }
        const monthlyTrend = Array.from(months.values()).sort((a, b) => a.ym.localeCompare(b.ym));
        res.json({
            data: {
...
router.get('/reports/filters', async (_req, res, next) => {
    try {
        ...
        res.json({
            data: {
                ...
                barangays: barangays.filter((b) => !!b && b.length > 0),
            },
        });
    }
    catch (err) {
