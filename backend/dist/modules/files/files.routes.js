...
const router = Router();
function canAccessFile(req, info) {
    if (!info || !req.auth)
        return false;
    if (req.auth.role === 'admin')
        return true;
    const metadata = info.metadata ?? {};
    const sensitivePurpose = ['face_capture', 'proof_of_residency', 'valid_id', 'credential'].includes(metadata.purpose ?? '');
    if (!sensitivePurpose)
        return true;
    if (!metadata.ownerId)
        return false;
    if (metadata.ownerType !== req.auth.role)
        return false;
