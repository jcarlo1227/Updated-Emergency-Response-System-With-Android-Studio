...
router.post('/register/user', async (req, res) => {
    try {
        ...
        await user.save();
        // Generate token
        const token = jwt.sign({ userId: user._id, email: user.email, type: 'user' }, JWT_SECRET, { expiresIn: '7d' });
        res.status(201).json({
...
router.post('/register/responder', async (req, res) => {
    try {
        ...
        // Generate token
        const token = jwt.sign({ responderId: responder._id, email: responder.email, type: 'responder' }, JWT_SECRET, { expiresIn: '7d' });
        res.status(201).json({
...
router.put('/user/:id/contacts', async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { contacts }, { new: true }).select('-password');
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
