...
const UNIT_TYPES = ['bls', 'als', 'patient_transport', 'rescue', 'other'] as const;
const AVAILABILITY = ['available', 'assigned', 'maintenance', 'out_of_service'] as const;
...
async function nextUnitNumber(): Promise<number> {
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const body = req.validated!.body as z.infer<typeof createUnitSchema>;
      ...
      try {
        ...
        if (err instanceof Error && 'code' in err && (err as { code?: number }).code === 11000) {
          throw new AppError(
            `Ambulance unit ${unitNumber} already exists`,
            409,
            'UNIT_EXISTS',
          );
        }
...
router.patch(
  '/ambulance-units/:id',
  validateRequest({ params: idParamSchema, body: updateUnitSchema }),
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { id } = req.validated!.params as { id: string };
      const body = req.validated!.body as z.infer<typeof updateUnitSchema>;
      ...
      if (body.unitName !== undefined) unit.unitName = body.unitName;
      if (body.plateNumber !== undefined) unit.plateNumber = body.plateNumber;
      if (body.unitType !== undefined) unit.unitType = body.unitType;
      if (body.availabilityStatus !== undefined) unit.availabilityStatus = body.availabilityStatus;
      ...
      if (body.unitNumber !== undefined && body.unitNumber !== unit.unitNumber) {
        ...
        if (conflict) throw new AppError('Unit number already in use', 409, 'UNIT_EXISTS');
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (unit.activeRequestId) {
        throw new AppError(
          'Cannot delete a unit currently assigned to a request. Mark inactive instead.',
          409,
          'UNIT_IN_USE',
        );
      }
