...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const q = req.validated!.query as z.infer<typeof auditQuerySchema>;
      ...
      let enriched = rawItems.map((log) => {
        const h = humanizeAudit(log as never);
        return { ...log, module: h.module, description: h.description, severity: h.severity };
      });
      if (q.module) enriched = enriched.filter((l) => l.module === q.module);
      if (q.search) {
...
router.get(
  ...
  async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const activeStatuses = ['pending', 'assigned', 'responder_on_the_way', 'responder_nearby', 'arrived'] as const;
      ...
      res.json({
        data: {
          ...
          activeResponders,
          emergenciesByType: Object.fromEntries(byType.map((b: { _id: string; count: number }) => [b._id, b.count])),
        },
      });
    } catch (err) { next(err); }
  },
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const responseAgg = await Emergency.aggregate<{ avgMs: number; count: number }>([
      ...
      const systemStatus = {
        ...
        socket: io ? ('connected' as const) : ('disconnected' as const),
        map: 'active' as const,
      };
